...
metadata:
  ...
  version: 1.1 — 디벨롭 레이어(훅·가독성·리듬·사운드·엔딩) + Higgs 플레이북
---
...
### Phase 2b — 디벨롭 규칙 적용 (v1.1, `references/develop-hook-readability.md`)
매니페스트를 쓰기 전에 훅·가독성 규칙을 데이터에 반영한다: `hook.coldOpen`(결과 수치 3장) · 첫 수치 문장에 그래픽 · `promiseCard` · 설명 씬 `chapterCard` · 얼굴 구간 `keywordChip` · `recapCard` · 마지막 15초 `endScreen` · `audio`(SFX/BGM). Higgs 확장(영상 B-roll·레퍼런스 분석·썸네일·쇼츠)은 `references/higgs-playbook.md`.
...
node scripts/build-timeline.mjs # 세그먼트·씬·비트·자막 큐(문장 병합→34폭 분할→6자 꼬리 흡수→2줄 균형→3.5s 상한) → data/timeline.json + 게이트 출력
node scripts/develop-audit.mjs  # 훅·가독성·리듬 채점(≥80) + 훅 비트 삽입 제안 → out/develop-audit.json
```
...
- 상황 B-roll은 영상(5~8초)으로 승격 가능 — `references/higgs-playbook.md`. 사운드는 SFX 자동 매핑 + BGM 슬롯(`scenes.audio`).
