...
export const Captions = () => {
  ...
  const pip = isPip(sc.layout);
  const centerX = sc.endScreen ? END.x + END.w / 2 : pip ? CAPTION.centerXWithPip : CAPTION.centerX;
...
const Highlight: React.FC<{ line: string; keyword: string | null }> = ({ line, keyword }) => {
  ...
  if (at < 0) return <>{line}</>;
  return (<>{line.slice(0, at)}<span style={{ color: C.coral, fontWeight: 800, fontSize: "1.15em" }}>{keyword}</span>{line.slice(at + keyword.length)}</>);
};
